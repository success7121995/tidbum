...
const MediaLibrary = ({ visible, onClose, onSelect, albumId }: MediaLibraryProps) => {
    ...
    // GESTURE PROVIDER
    // ============================================================================
    const { createSwipeSelectionPanResponder } = useGesture();
    ...
    const panResponder = useMemo(() => {
        const gestureConfig = {
            ...
            itemSize,
            gap,
            offsetY: insets.top + 40, // Account for header height
            offsetX: gap,
            scrollOffsetX,
            scrollOffsetY,
            componentType: 'mediaLibrary' as const,
        };
        ...
        return createSwipeSelectionPanResponder(
            ...
            selectedAssetIds,
            isScrolling,
            true, // Always in selection mode for MediaLibrary
            {
                ...
                onHoverChange: (assetId: string | null) => {
                    setCurrentHoverAssetId(assetId);
                },
                onSwipeSelectingChange: (isSelecting: boolean) => {
                    setIsSwipeSelecting(isSelecting);
                },
            }
        );
    }, [isScrolling, selectedAssetIds, filteredAssets, createSwipeSelectionPanResponder, numColumns, itemSize, gap, insets.top, scrollOffsetX, scrollOffsetY]);
